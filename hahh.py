...
BOT_TOKEN = "8066456754:AAHNihOcYiHa520OUfsLyl74GIBYQdunQ8I"
...
merge_queue = {}
split_queue = {}
rename_queue = {} # Handling both rename modes here for stability
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    welcome = (
        "🔥 **Advance VCF Manager Bot** 🔥\n\n"
        "Main aapka contact management asaan bana sakta hoon.\n"
        "Niche diye gaye option se shuru karein: 👇"
    )
...
async def buttons(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "main_menu":
        st.clear()
        return await q.message.edit_text("👋 Choose an option niche se:", reply_markup=main_menu())

    if q.data == "gen":
        st.update({"mode": "gen", "step": "file_name"})
        await q.message.edit_text("1️⃣ **Step 1:** VCF File Name bhejiye:", reply_markup=back_kb())

    elif q.data == "gen_done":
        st["step"] = "waiting_input"
        await q.message.edit_text("✅ Settings saved! Ab apni **TXT file** bhejiye.", reply_markup=back_kb())
        ...
        await q.message.edit_text("📥 Files bhejein, phir **DONE** likhein.", reply_markup=back_kb())

    elif q.data == "rename_files":
        st.update({"mode": "rename_files", "step": "file"})
        rename_queue[uid] = []
        await q.message.edit_text("📂 Send VCF file jiska **Filename** badalna hai:", reply_markup=back_kb())

    elif q.data == "rename_contacts":
        st.update({"mode": "rename_contacts", "step": "file"})
        rename_queue[uid] = []
        await q.message.edit_text("📂 Send VCF file jiske **Contacts** rename karne hain:", reply_markup=back_kb())
        ...
        await q.message.reply_text("✅ Merged Successfully!", reply_markup=main_menu())

    elif q.data == "quick_vcf":
        st.update({"mode": "quick", "step": "file"})
        await q.message.edit_text("📂 **Quick VCF:** Send File Name:", reply_markup=back_kb())

    elif q.data == "skip_cc":
        cfg["country_code"] = ""; st["step"] = "group_number"
        await q.message.edit_text("7️⃣ Group Name (Optional):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏩ Skip", callback_data="skip_group")]]))
        ...
        await q.message.edit_text(f"📤 Mode: {q.data.replace('_',' ').upper()}\nSend the file:", reply_markup=back_kb())
        ...
        await q.message.edit_text("♻️ Reset Done!", reply_markup=main_menu())
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if st["mode"] == "gen":
        if st["step"] == "file_name":
            cfg["file_name"] = txt; st["step"] = "contact_name"; await update.message.reply_text("2️⃣ Contact Name?")
        elif st["step"] == "contact_name":
            cfg["contact_name"] = txt; st["step"] = "limit"; await update.message.reply_text("3️⃣ Limit per file?")
        elif st["step"] == "limit":
            cfg["limit"] = int(txt) if txt.isdigit() else 100; st["step"] = "contact_start"; await update.message.reply_text("4️⃣ Contact Start?")
        elif st["step"] == "contact_start":
            cfg["contact_start"] = int(txt) if txt.isdigit() else 1; st["step"] = "vcf_start"; await update.message.reply_text("5️⃣ VCF Start?")
        elif st["step"] == "vcf_start":
            cfg["vcf_start"] = int(txt) if txt.isdigit() else 1; st["step"] = "country_code"
            await update.message.reply_text("6️⃣ Country Code:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏩ Skip", callback_data="skip_cc")]]))
        elif st["step"] == "country_code":
            cfg["country_code"] = txt; st["step"] = "group_number"
            await update.message.reply_text("7️⃣ Group Name:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏩ Skip", callback_data="skip_group")]]))
        ...
        if st["step"] == "file":
            st["file"] = txt; st["step"] = "contact"; await update.message.reply_text("👤 Contact Name?")
        elif st["step"] == "contact":
            st["contact"] = txt; st["step"] = "numbers"; await update.message.reply_text("📤 Send numbers:")
...
async def handle_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if st["mode"] == "split":
        ...
        await update.message.reply_text(f"🔢 Total {len(nums)} numbers. Per file limit?")
        ...
        await update.message.reply_text(f"✏️ Send {prompt}")
        ...
        os.remove(path); st.clear(); await update.message.reply_text("✅ Complete!", reply_markup=main_menu())
        ...
        os.remove(f); os.remove(path); st.clear(); await update.message.reply_text("✅ Done!", reply_markup=main_menu())
        ...
        merge_queue[uid].append(path); await update.message.reply_text(f"📥 `{path}` added. More or DONE.")

    elif st["mode"] == "file_count":
        n = extract_vcf(path) if path.endswith(".vcf") else extract_txt(path)
        await update.message.reply_text(f"📊 Numbers: `{len(n)}`", reply_markup=main_menu()); os.remove(path); st.clear()
